...
st.markdown("<p style='text-align:center;'>Compare algorithms, visualize matches, and generate a research-ready report</p>", unsafe_allow_html=True)
...
if st.button("🚀 Run Comparison"):
    if not dna_sequence or not pattern:
        ...
        st.markdown("### 🧬 Genome Track")
        fig2, ax2 = plt.subplots(figsize=(10, len(results)))
        for i, algo in enumerate(results):
            y = len(results) - i
            for pos in algo["Positions"]:
                rect = patches.Rectangle((pos, y-0.3), len(pattern), 0.6, color="#00B4D8", alpha=0.7)
                ax2.add_patch(rect)
        ax2.set_xlim(0, len(dna_sequence))
        ax2.set_yticks(range(1, len(results)+1))
        ax2.set_yticklabels([r["Algorithm"] for r in results])
        ax2.set_xlabel("Sequence Position (bp)")
        ax2.set_title("Match Locations")
        buf2 = io.BytesIO()
        ...
        if st.button("📄 Export PDF Report"):
            ...
            content = [
                Paragraph("<b>DNA Pattern Matching Report</b>", styles["Title"]),
                Spacer(1, 12),
                Paragraph(f"<b>Pattern:</b> {pattern}", styles["Normal"]),
                Paragraph(f"<b>Sequence Length:</b> {len(dna_sequence)} bp", styles["Normal"]),
                Spacer(1, 12),
                Paragraph("<b>Results:</b>", styles["Heading2"])
            ]
            for i, row in df.iterrows():
                content.append(Paragraph(f"{row['Algorithm']}: {row['Matches']} matches, Time: {row['Time (s)']} s", styles["Normal"]))
            Spacer(1, 12)
            buf.seek(0); buf2.seek(0)
            content += [Paragraph("<b>Performance Chart</b>", styles["Heading2"]), Image(buf, width=400, height=250),
                        Paragraph("<b>Genome Track Visualization</b>", styles["Heading2"]), Image(buf2, width=400, height=250)]
            doc.build(content)
            st.download_button("⬇ Download PDF Report", pdf_buffer.getvalue(), "DNA_Report.pdf", "application/pdf")
...
if st.button("📚 Fetch Papers"):
    query = "DNA sequence pattern matching algorithm"
    url = f"https://api.semanticscholar.org/graph/v1/paper/search?query={query}&limit=3&fields=title,authors,year,url"
    try:
        ...
        if response.status_code == 200:
            ...
            for paper in data.get("data", []):
                ...
                st.markdown(f"**[{paper['title']}]({paper['url']})**  \n📅 {paper.get('year', 'N/A')} | 👩‍🔬 {authors}")
        else:
            st.error("Error fetching papers.")
    except Exception:
        st.error("⚠️ Could not fetch research papers. Check connection.")
